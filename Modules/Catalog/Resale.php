...
class Resale extends CatalogPublic
{
    ...
    public function items() {
		...
		if($request!='/resale/' and !empty($request_params)){
			if($request_params[0]=='price_1' or $request_params[0]=='petrodvortsovyj-'){
        ...
        if (!empty($title_search)) {
			//$title_search = explode(' ',$title_search)[0];
            $kb_phrase = \LPS\Components\FormatString::keyboardLayout($title_search, 'both');
			
			foreach(explode(' ',$title_search) as $title){ $title = str_replace(array(',','.'),'',$title);
				$search_params[] = RuleAggregator::make(RuleAggregator::LOGIC_OR, array(
						Rule::make(RealEstateConfig::KEY_APPART_ADDRESS)->setValue($title, Rule::SEARCH_LIKE),
