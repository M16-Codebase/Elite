	...
	">
						...
						{foreach from = $segments item=$s}
							{if $s.key == $constants.segment_default_key}
								<div class="lang-col w6">
									<input type="radio" name="{$property.key}" value=""{if !empty($prop_val_id)} data-val-id="{$prop_val_id}"{/if}{if empty($prop_val)} checked{/if} />
							...
							{foreach from = $segments item=$s}
								{if $s.key == $constants.segment_default_key}
									<div class="lang-col w6">
										...
										<span class="a-inline-block">{if !empty($enum_val.value)}{$enum_val.value}{else}{$enum_val}{/if}</span>
									</div>
								{else}
									<div class="lang-col {$s.key}-col w6">
										<span class="{$s.key}-var a-hidden">
											{!empty($property['segment_enum'][$enum_id][$s.id]) ? $property['segment_enum'][$enum_id][$s.id] : $enum_id}
										</span>
									</div>
								{/if}
							{/foreach}
						...
						{if $s.key == $constants.segment_default_key}
							<div class="lang-col w6">
								...
								<div class="range-vals">
									{?$input_string = $property['values'][$s.id]['min_max']|replace:ldelim . "min" . rdelim:$prop_range_min|replace:ldelim . "max" . rdelim:$prop_range_max|html}
		...
		{elseif $property.data_type == 'address'}
			<div class="field m-address">
				<div class="row">
				{foreach from = $segments item=$s}	
					<div class="lang-col{if $s.key != $constants.segment_default_key} {$s.key}-col{/if} {if $property.set == 1}{if $s.key == $constants.segment_default_key}w55{else}w5{/if}{else}w6{/if}">
						<input type="text" name="{$property.key}"{if $s.key == $constants.segment_default_key} class="address-input"{/if}{if $property.set != 1} data-val-id="{$lang_values[$s.key][$property.key]['val_id']}"{/if}{if $property.set != 1} value="{$lang_values[$s.key][$property.key]['value']}"{/if}  data-segment="{$s.id}" />
					</div>
				{/foreach}
				</div>
				<div class="row">
		...
		{elseif $property.data_type == 'date' || $property.data_type == 'color'}	
			...
			{if $property.set == 1}
				...
				{/if}
					<div class="add-row row m-fullwidth unchangeable">
						<div class="add-button add-btn w3" data-entity_id="2" data-property_id="634" data-segment_id="0">
							<i class="icon-add"></i> <span class="small-descr">Добавить</span>
						</div>
						<div class="w8"></div>
						<div class="w1">
		...
		{elseif $property.data_type == 'text'}
			...
			{foreach from = $segments item=$s}
				<div class="lang-col{if $s.key != $constants.segment_default_key} {$s.key}-col{/if} {if $property.set == 1}{if $s.key == $constants.segment_default_key}w55{else}w5{/if}{else}w6{/if}">
					<textarea name="{$property.key}" {if $property.set != 1} data-val-id="{$lang_values[$s.key][$property.key]['val_id']}"{/if} data-segment="{$s.id}">
						{if $property.set != 1}
							{$lang_values[$s.key][$property.key]['value']}
						{/if}
					</textarea>
				</div>
			{/foreach}
				...
				{if !empty($prop_val_ids_by_pos)}
					{foreach from=$prop_val_ids_by_pos item=val_ids key=pos}								
						<div class="multi-item row">
							<div class="drag-drop w05"></div>
							{foreach from = $segments item=$s}
								<div class="lang-col {if $s.key != $constants.segment_default_key}{$s.key}-col w5{else}w55{/if}">
									{?$segment_field_val_id = !empty($prop_val_ids_by_pos[$pos][$s.key])? $prop_val_ids_by_pos[$pos][$s.key] : null}
									<textarea name="{$property.key}"{if !empty($segment_field_val_id)} data-val-id="{$segment_field_val_id}"{/if} data-segment="{$s.id}">
										{if !empty($lang_values[$s.key][$property.key]['value'][$segment_field_val_id])}
											{$lang_values[$s.key][$property.key]['value'][$segment_field_val_id]}
										{/if}
									</textarea>
								</div>
							{/foreach}
		...
		{else}
			...
			{foreach from = $segments item=$s}
				<div class="lang-col {if $s.key != $constants.segment_default_key}{$s.key}-col{/if} {if $property.set == 1}{if $s.key == $constants.segment_default_key}w55{else}w5{/if}{else}w6{/if}">
					<input type="text" name="{$property.key}"
						{if $property.set != 1} value="{$lang_values[$s.key][$property.key]['value']}"{else}value=""{/if}
						{if $property.set != 1} data-val-id="{$lang_values[$s.key][$property.key]['val_id']}"{/if}
					...
					{foreach from=$prop_val_ids_by_pos item=val_ids key=pos}
						<div class="multi-item row">
							...
							{foreach from = $segments item=$s}
								<div class="lang-col {if $s.key != $constants.segment_default_key}{$s.key}-col{/if} {if $property.set == 1}{if $s.key == $constants.segment_default_key}w55{else}w5{/if}{else}w6{/if}">
									{?$segment_field_val_id = !empty($prop_val_ids_by_pos[$pos][$s.key])? $prop_val_ids_by_pos[$pos][$s.key] : null}
									<input type="text" name="{$property.key}" class="title"
										{if !empty($lang_values[$s.key][$property.key]['value'][$segment_field_val_id])} value="{$lang_values[$s.key][$property.key]['value'][$segment_field_val_id]}"{/if}
										{if !empty($segment_field_val_id)} data-val-id="{$segment_field_val_id}"{/if}
										{if $property.data_type == 'int'} data-mask="?9999999999"{/if}
										{if $property.read_only == 1} disabled{/if}
										data-segment="{$s.id}" />
								</div>
