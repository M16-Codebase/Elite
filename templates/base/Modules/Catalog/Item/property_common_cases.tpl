<div class="wblock prop-item-cont white-block-row" data-prop-key="{$property.key}" data-prop-id="{$property.id}"{if !empty($property.context)} data-context="{$property.context}"{/if}>
					...
					{foreach from = $segments item=$s}
						{if $s.key == $constants.segment_default_key}
							<div class="lang-col w6">
                                ...
                                {if (isset($property['values']['min_max']))}
								<div class="range-vals">
									{$property['values']['min_max']|replace:ldelim . "min" . rdelim:$prop_range_min|replace:ldelim . "max" . rdelim:$prop_range_max|html}
									{if !empty($property.mask)}
										{$property.mask|replace:ldelim . "!" . rdelim:''}
									{/if}
								</div>
                                {/if}
							</div>
						{else}
							<div class="lang-col {$s.key}-col w6">
								<div class="min-val range-val a-hidden {$s.key}-var">
									{$property['values']['min']|replace:ldelim . "min" . rdelim:'<span class="combo-min cmb"></span>'|html}
									{if !empty($property.mask)}
										{$property.mask|replace:ldelim . "!" . rdelim:''}
									{/if}
								</div>
								<div class="max-val range-val a-hidden {$s.key}-var">
									{$property['values']['max']|replace:ldelim . "max" . rdelim:'<span class="combo-max cmb"></span>'|html}
									{if !empty($property.mask)}
										{$property.mask|replace:ldelim . "!" . rdelim:''}
									{/if}
								</div>
								...
								<div class="two-val range-val a-hidden {$s.key}-var">
									{$property['values']['min_max']|replace:ldelim . "min" . rdelim:'<span class="cmb cmb1"></span>'|replace:ldelim . "max" . rdelim:'<span class="cmb cmb2"></span>'|html}
									{if !empty($property.mask)}
										{$property.mask|replace:ldelim . "!" . rdelim:''}
									{/if}
								</div>
		...
		{elseif !empty($property.mask) && $property.mask != ldelim . "!" . rdelim}
			...
			<div class="{if $property.set == 1}w10{else}field{/if}">
				...
				{if $property.set != 1 && !empty($prop_val_id)}
					{?$input_string .= 'value="' . $prop_val . '" '}
					{?$input_string .= 'data-val-id="' . $prop_val_id . '" '}
				{/if}
				{if $property.data_type == 'int'}{?$input_string .= 'data-mask="?9999999999" '}{/if}
				{?$input_string .= '/>'}
				{?$prop_unit = $property['mask']|replace:ldelim . "!" . rdelim:''}
				{?$prop_string = $property['mask']|replace:$prop_unit:('<span class="prop-unit">' . $prop_unit . '</span>')}
			...
			{if $property.set == 1}
				...
				{if !empty($prop_val_id)}
					{foreach from=$prop_val_id item=val_id}
						<div class="multi-item combo-cont row">
							<div class="drag-drop w05"></div>
							<div class="field w10">
								{?$input_string = '<input type="text" '}
								{?$input_string .= 'value="' . $prop_val[$val_id] . '" '}
								{?$input_string .= 'data-key="' . $property.key . '" '}
								{?$input_string .= 'name="' . $property.key . '" '}
								{?$input_string .= 'data-val-id="' . $val_id . '" '}
								{if $property.data_type == 'int'}{?$input_string .= 'data-mask="?9999999999" '}{/if}
								{?$input_string .= '/>'}
								{?$prop_unit = $property['mask']|replace:ldelim . "!" . rdelim:''}
								{?$prop_string = $property['mask']|replace:$prop_unit:('<span class="prop-unit">' . $prop_unit . '</span>')}
