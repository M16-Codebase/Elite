...
function pano2vrPlayer(h,baseUrl) {
	function q(a) {
		var d, c;
		c = [];
	...
	function B(a) {
		...
		for (d = 0; d < y.length; d++)
			if (y[d].id == a) return y[d];
		for (d = 0; d < fa.length; d++)
			if (fa[d].id == a) return fa[d];
		return o
	}
	...
	function rb() {
		...
		for (var d = 0; d < C.length + y.length; d++) {
			...
			if (c.obj) {
				...
				5 == c.mode && (b = 180 * Math.acos(a.wa(c.n)) / Math.PI, b < c.s ? c.obj.volume = c.i * Y : (b -= c.s, b < F && 0 < F ? (h =
					1 - Math.abs(b / F), c.obj.volume = c.i * (g + (1 - g) * h) * Y) : c.obj.volume = g * Y));
	...
	function Da() {
		...
		xb && (b.changeFov(0.4 *
			(V - A)), 0.001 > Math.abs(V - A) / A && (xb = r), b.update());
		...
		if (zb) {
			...
			if (d = a.getTime() - jb, 0 < mc && b.Ba && d >= 1E3 * mc) {
				if (1 < va.length) {
					if (Rc) {
						d = 1E3;
						do c = va[Math.floor(Math.random() * va.length)]; while (d-- && c == Jb)
			...
			if (Ab && 0 > Q && a.getTime() -
				lc > 1E3 * nc && (oc && b.isLoaded || !oc)) ia = l, jb = a.getTime(), N = O = G = 0;
		...
		if (0 < y.length)
			for (d = 0; d < y.length; d++) y[d].Ra && y[d].$a != y[d].obj.currentTime && (y[d].$a = y[d].obj.currentTime, !y[d].Va && 0 < y[d].obj.videoHeight && (y[d].Va = y[d].obj.videoWidth /
				y[d].obj.videoHeight));
		if (0 < R) {
			if (2 == R)
				for (d = 0; d < J.length; d++) a = J[d], "poly" == a.type && a.C != a.p && (a.C > a.p ? (a.p += 0.05, a.C < a.p && (a.p = a.C)) : (a.p -= 0.05, a.C > a.p && (a.p = a.C)), b.update());
	...
	function jd(a) {
		...
		if (!ca && (0 <= Q && (a.preventDefault(), (a.which || 0 == a.which || 1 == a.which) && ad(a.pageX, a.pageY), K()), b.hotspot == b.emptyHotspot || "poly" == b.hotspot.type)) {
			...
			b.skinObj.hotspotProxyOut(b.hotspot.id)), c ? (b.hotspot = c, b.skinObj && b.skinObj.hotspotProxyOver && b.skinObj.hotspotProxyOver(b.hotspot.id), S.style.cursor = "pointer", 0 < R && (ya = 1, b.hotspot.C = 1)) : (b.hotspot = b.emptyHotspot, S.style.cursor = "auto", 0 < R && (ya = 0)));
	...
	function Wc(a, d) {
		var c = Sb;
		c.enabled && (b.hotspot != b.emptyHotspot && 0 <= a && 0 <= d && "" != b.hotspot.title ? (M.innerHTML = b.hotspot.title, M.style.color = Ia(c.Ea, c.Da), M.style.backgroundColor = c.background ? Ia(c.P, c.O) : "transparent", M.style.border =
			"solid " + Ia(c.R, c.Q) + " " + c.ya + "px", M.style.borderRadius = c.xa + "px", M.style.textAlign = "center", 0 < c.width ? (M.style.left = a - c.width / 2 + Z + "px", M.style.width = c.width + "px") : (M.style.width = "auto", M.style.left = a - M.offsetWidth / 2 + Z + "px"), M.style.height = 0 < c.height ? c.height + "px" : "auto", M.style.top = d + 25 + +ba + "px", M.style.visibility = "inherit", M.style.overflow = "hidden") : (M.style.visibility = "hidden", M.innerHTML = ""))
	}

	function kd(a) {
		var d = Db();
		Ob() ? (b.mouse.x = a.clientX - Z, b.mouse.y = a.clientY - ba) : (b.mouse.x = a.pageX - d.x,
	...
	function nd() {
		...
		for (c = 0; 6 > c; c++) Wb[c] && (a = new Image, a.crossOrigin = b.ba, a.src = pa(Wb[c]), a.addEventListener ? a.addEventListener("load",
			Bc(da[c]), r) : a.onload = Bc(da[c]), da[c].ra = a, b.checkLoaded.push(a));
	...
	function pa(a) {
		...
		0 < a.indexOf("://") || 0 == a.indexOf("javascript:") ? a : Eb + a : Eb
	}
	...
	function rd() {
		for (var a = 0, d = Math.tan(b.getVFov() * Math.PI /
			360), c = t / (2 * d), c = c * (1 + d * (z / t) / 2), c = c * Math.pow(2, sd); D.length >= a + 2 && !D[a + 1].Ta && D[a + 1].width > c;) a++;
		return a
	}
	...
	function ud() {
		...
		for (a = 0; 6 > a; a++) {
			...
			if (d.F) {
				...
				for (var f = 0; f < c.length; f++) 4 > a ? c[f].l(a * (-Math.PI / 2)) : c[f].k((4 == a ? -1 : 1) * (Math.PI / 2)), c[f].l(-u *
					Math.PI / 180), c[f].k(x * Math.PI / 180), c[f].B(la * Math.PI / 180);
				c = Tb(c);
				for (var e = {
					ma: 1,
					na: 1,
	...
	function vd(a) {
		...
		c.V = Math.min(Math.max(0, Math.floor(c.V)),
			d.aa - 1);
	...
	function Od(a) {
		return function() {
			...
			0 == U && b.divSkin && b.divSkin.ggLoadedLevels && b.divSkin.ggLoadedLevels();
			try {
				g.pixelStorei(g.UNPACK_FLIP_Y_WEBGL, l), a.a != o && a.a.complete && (a.M = g.createTexture(), g.bindTexture(g.TEXTURE_2D, a.M), g.texImage2D(g.TEXTURE_2D, 0, g.RGBA, g.RGBA, g.UNSIGNED_BYTE, a.a), g.texParameteri(g.TEXTURE_2D, g.TEXTURE_MAG_FILTER, g.LINEAR), g.texParameteri(g.TEXTURE_2D,
					g.TEXTURE_MIN_FILTER, g.LINEAR), g.texParameteri(g.TEXTURE_2D, g.TEXTURE_WRAP_S, g.CLAMP_TO_EDGE), g.texParameteri(g.TEXTURE_2D, g.TEXTURE_WRAP_T, g.CLAMP_TO_EDGE), g.bindTexture(g.TEXTURE_2D, o))
			} catch (d) {}
			b.update()
		}
	}
	...
	function Ua() {
		...
		2 * d > Ka - La && (d = (Ka - La) / 2);
		b.setVFov(2 * d);
		90 > Ka ? x + d > Ka && (x = Ka - d) : x > Ka && (x = Ka); - 90 < La ? x - d < La && (x =
			La + d) : x < La && (x = La);
		if (359.99 > hb - ib) {
			var c = 0;
			if (0 != x) {
	...
	function Oc() {
		for (var a = new P(0, 0, -100), d = b.S(), c = 0; c < J.length; c++) {
			...
			if ("point" == f.type) {
				...
				a.B(la * Math.PI / 180);
				var h = r;
				0.01 > a.e ? (g = -d / a.e, e = a.x * g, g *= a.y, Math.abs(e) < z / 2 + 500 && Math.abs(g) <
				t / 2 + 500 && (h = l)) : g = e = 0;
				f.obj && f.obj.__div && (f.obj.__div.style[xa] = "none", f.obj.ggUse3d ? (ma || qd(d), f.obj.__div.style.width = "1px", f.obj.__div.style.height = "1px", hs = "", ma && (hs += "perspective(" + d + "px) "), hs += "translate3d(0px,0px," + d + "px) ", hs += "rotateZ(" + la.toFixed(10) + "deg) ", hs += "rotateX(" + x.toFixed(10) + "deg) ", hs += "rotateY(" + (-u).toFixed(10) + "deg) ", hs += "rotateY(" + f.pan.toFixed(10) + "deg) ", hs += "rotateX(" + (-f.tilt).toFixed(10) + "deg) ", hs += "translate3d(0px,0px," + (-1 * f.obj.gg3dDistance).toFixed(10) + "px) ",
					f.obj.__div.style[T + "Origin"] = "0% 0%", f.obj.__div.style[T] = hs, f.obj.__div.style.left = Z + z / 2 + "px", f.obj.__div.style.top = ba + t / 2 + "px") : h ? (f.obj.__div.style.left = Z + e + z / 2 + "px", f.obj.__div.style.top = ba + g + t / 2 + "px") : (f.obj.__div.style.left = "-1000px", f.obj.__div.style.top = "-1000px"))
			}
			if ("poly" == f.type) {
				...
				if (0 < k.length)
					for (h =
							 0; h < k.length; h++) a = k[h], 0.1 > a.e ? (g = -d / a.e, e = z / 2 + a.x * g, g = t / 2 + a.y * g) : g = e = 0, a.$ = e, a.G = g;
				f.ua = k
			}
		}
	}
	...
	function Pd(a, b) {
		...
		var e = Sb;
		e.enabled && (c.text = document.createElement("div"), f = "position:absolute;" + ("left: -" +
	...
	b.detectBrowser = function() {
		...
		Dd("Pano2VR player - Prefix:" + I + ", " + (Gd ? "CSS 3D available" : "CSS 3D not available") + ", " + (ga ? "WebGL available" :
				"WebGL not available"))
	};
	...
	b.setViewerSize = function(a, d) {
		...
		if (!(10 > c || 10 > f)) {
			...
			na && (na.style.width = a + "px", na.style.height = d + "px",
				na.width = a, na.height = d);
	...
	b.setVFov = function(a) {
		...
		switch (Kc) {
			case 0:
				...
				break;
			case 1:
				a = 180 * Math.atan(z / t * Math.tan(a * Math.PI /
							180)) / Math.PI;
				A = 2 * a;
				break;
			case 2:
	...
	b.updatePanorama = function() {
		...
		Cd.q(0, -c, -f);
		Oc();
		if ($ && (Hc != R && (Hc = R, $.style.visibility = 0 <
			R ? "inherit" : "hidden"), 0 < R)) {
			W || (W = $.getContext("2d"));
			...
			for (f = 0; f < J.length; f++)
				if (d = J[f], "poly" == d.type && (c = d.ua, 2 == R && (a = d.p), W.fillStyle = Ia(d.P, d.O * a), W.strokeStyle = Ia(d.R, d.Q * a), 0 < c.length)) {
					W.beginPath();
		...
		if (ga)
				...
				if (g) {
					...
					for (h = D.length - 1; h >= e;) {
						...
						for (curCfNr = 0; 6 > curCfNr; curCfNr++)
							if (f = b.f.j[curCfNr], d = f.ta, ua(ea), Ib(ea, -la * Math.PI / 180, [0, 0, 1]), Ib(ea, -x * Math.PI / 180, [1, 0, 0]), Ib(ea, (180 - u) * Math.PI / 180, [0, 1, 0]), 4 > curCfNr ? Ib(ea, -Math.PI / 2 * curCfNr, [0, 1, 0]) : Ib(ea,
									Math.PI / 2 * (5 == curCfNr ? 1 : -1), [1, 0, 0]), f.F && d && 0 < d.Ga && 0 < d.Ka && 0 < d.scale || level.t) {
								f.dirty = r;
								level.t ? (c = {
									U: 0,
									V: 0
								...
								for (ty = c.V; ty <= c.Z; ty++)
									for (tx = c.U; tx <= c.Y; tx++) {
										...
										U < Zb && !d.a && (Fd++, d.a = new Image, d.a.onload = Od(d), d.a.onerror = xd(), d.a.onabort = xd(), d.a.crossOrigin = b.ba, d.a.setAttribute("src", od(h)), level.t && b.checkLoaded.push(d.a), 0 == U && b.divSkin && b.divSkin.ggReLoadedLevels && b.divSkin.ggReLoadedLevels(),
											U++, b.dirty = l);
										if (d.M) {
			...
			if (0 < D.length) {
				...
				if (0 < D.length) {
					...
					for (c = D.length - 1; c >= d;) {
						...
						for (curCfNr = 0; 6 > curCfNr; curCfNr++)
							if (f = b.f.j[curCfNr], k = f.ta, f.F && k && 0 < k.Ga && 0 < k.Ka && 0 < k.scale || level.t) {
								f.dirty = r;
								var p;
								level.t ? (p = {
									U: 0,
									...
									for (tx = p.U; tx <= p.Y; tx++) {
										...
										if (!k.b && U < Zb) {
											if (0 < unusedTileCanvas.length) {
												...
												for (q = s.firstChild; q && q.K && (-1 == q.K || q.K >=
												c);) q = q.nextSibling;
											...
											k.b && (Fd++, k.a = new Image, k.a.style[T + "Origin"] = "0% 0%", k.a.style.position = "absolute",
												k.a.style.overflow = "hidden", k.a.crossOrigin = b.ba, k.b.K = c, n && (k.b.id = "tile" + curCfNr + "_" + c + "___" + ty + "_" + tx), k.a.onload = Nd(k), k.a.onerror = wd(k), k.a.onabort = wd(k), k.a.setAttribute("src", od(c)), level.t && b.checkLoaded.push(k.a), 0 == U && b.divSkin && b.divSkin.ggReLoadedLevels && b.divSkin.ggReLoadedLevels(), U++, b.dirty = l)
										}
										k.b && (a = "", ma ? (a += "translate3d(" + z / 2 + "px," + t / 2 + "px,0px) ", a += " perspective(" + h + "px) ", a += "translate3d(0px,0px," + h + "px) ") : a += "translate3d(" + z / 2 + "px," + t / 2 + "px," + h + "px) ", a += "rotateZ(" + Number(la).toFixed(10) +
			...
			if (z != s.offsetWidth || t != s.offsetHeight) z = parseInt(s.offsetWidth), t = parseInt(s.offsetHeight);
			h && (e = h.canvas.width / 2, n = h.canvas.height / 2, a = h.createRadialGradient(e, n, 5, e, n, Math.max(e, n)), a.addColorStop(0, "#333"), a.addColorStop(1, "#fff"), h.rect(0, 0, h.canvas.width, h.canvas.height), h.fillStyle =
				a, h.fill(), h.fillStyle = "#f00", h.font = "20px Helvetica", h.textAlign = "center", h.fillText("Pan: " + u.toFixed(1), e, n - 30), h.fillText("Tilt: " + x.toFixed(1), e, n), h.fillText("Fov: " + A.toFixed(1), e, n + 30))
		}
		bc()
	};
	var ma = r,
	...
	b.initWebGL = function(a) {
		...
		try {
			if (Ba = a ? a : document.createElement("canvas"),
					Ba.width = 100, Ba.height = 100, s.appendChild(Ba), (g = Ba.getContext("webgl")) || (g = Ba.getContext("experimental-webgl")), g) {
				g.Ja = 500;
				g.Ia = 500;
	...
	b.Wa = function() {
		...
		a.addEventListener ? (a.addEventListener("touchstart", Md, r), a.addEventListener("touchmove", Ld, r), a.addEventListener("touchend", fb, r), a.addEventListener("touchcancel", eb, r), a.addEventListener("MSPointerDown", db, r), a.addEventListener("MSGestureStart", Oa, r), a.addEventListener("MSGestureEnd", Na, r), a.addEventListener("MSGestureChange", bb, r), a.addEventListener("gesturestart",
			...
			$a), window.attachEvent("onresize", Aa), window.attachEvent("onblur", Za));
	...
	b.addHotspotElements = function() {
		for (var a = 0; a < J.length; a++)
			if ("point" == J[a].type && (J[a].obj = b.skinObj && b.skinObj.addSkinHotspot ? new b.skinObj.addSkinHotspot(J[a]) : new Pd(this, J[a]), J[a].obj.__div.style.left = "-1000px", J[a].obj.__div.style.top = "-1000px", J[a].obj && J[a].obj.__div)) {
				var d = S.firstChild;
				d ? S.insertBefore(J[a].obj.__div,
					d) : S.appendChild(J[a].obj.__div)
			}
	};
	...
	b.setFullscreen = function(a) {
		...
		if (b.isFullscreen) {
			if (Yb) try {
				b.c.webkitRequestFullScreen ?
					b.c.webkitRequestFullScreen() : b.c.mozRequestFullScreen ? b.c.mozRequestFullScreen() : b.c.msRequestFullscreen ? b.c.msRequestFullscreen() : b.c.requestFullScreen ? b.c.requestFullScreen() : b.c.requestFullscreen && b.c.requestFullscreen()
			} catch (c) {}
	...
	b.createLayers = function(a) {
		...
		b.da ? (b.da.innerHTML = "", b.c = document.createElement("div"), d && b.c.setAttribute("id", "viewport"), a = "top:  0px;left: 0px;position:relative;-ms-touch-action: none;" +
				...
				for (a = 0; a < D.length; a++) {
					...
					for (c in d.g)
						if (d.g.hasOwnProperty(c)) {
							...
							f.ja = r;
							f.b &&
							(f.d && (f.d.clear ? f.d.clear() : f.d.clearRect(0, 0, f.d.canvas.width, f.d.canvas.height)), unusedTileCanvas.push(f.b));
							f.a && delete f.a;
	...
	b.readConfigUrl = function(a, d, c) {
		try {
			...
			if (f.responseXML) {
				...
				var trueText = f.responseText.replace(/\/images\//g, baseUrl + '/images/');
				b.readConfigString(trueText, c)
	...
	b.Ca = function(a, d) {
		...
		for (var c = a.firstChild, f, e, h, p = 0; c;) {
			if ("view" == c.nodeName) {
				(e = c.getAttributeNode("fovmode")) &&
				(Kc = Number(e.nodeValue));
				...
				for (f = c.firstChild; f;) "start" == f.nodeName && (e = f.getAttributeNode("pan"), Ic = u = Number(e ? e.nodeValue : 0), e = f.getAttributeNode("tilt"), Jc = x = Number(e ? e.nodeValue : 0), e = f.getAttributeNode("fov"), Mb = A = Number(e ? e.nodeValue : 70)), "min" == f.nodeName && (e = f.getAttributeNode("pan"), ib = 1 * (e ? e.nodeValue : 0), e = f.getAttributeNode("tilt"), La = 1 * (e ? e.nodeValue : -90), e = f.getAttributeNode("fov"), ka = 1 * (e ? e.nodeValue : 5), 1.0E-20 > ka && (ka = 1.0E-20), e =
					f.getAttributeNode("fovpixel"), Cc = 1 * (e ? e.nodeValue : 0)), "max" == f.nodeName && (e = f.getAttributeNode("pan"), hb = 1 * (e ? e.nodeValue : 0), e = f.getAttributeNode("tilt"), Ka = 1 * (e ? e.nodeValue : 90), e = f.getAttributeNode("fov"), sa = 1 * (e ? e.nodeValue : 120), 180 <= sa && (sa = 179.9)), f = f.nextSibling
			}
			if ("autorotate" == c.nodeName && ((e = c.getAttributeNode("speed")) && (Ra = 1 * e.nodeValue), (e = c.getAttributeNode("delay")) && (nc = 1 * e.nodeValue), (e = c.getAttributeNode("returntohorizon")) && (Lb = 1 * e.nodeValue), (e = c.getAttributeNode("nodedelay")) &&
				(mc = 1 * e.nodeValue), (e = c.getAttributeNode("noderandom")) && (Rc = 1 == e.nodeValue), $b && (ia = Ab = l, jb = (new Date).getTime()), e = c.getAttributeNode("startloaded")))(oc = 1 == e.nodeValue) && (ia = r);
			"input" == c.nodeName && (h || (h = c));
			if (h)
				for (f = 0; 6 > f; f++) e = h.getAttributeNode("prev" + f + "url"), mb[f] = e ? new String(e.nodeValue) : "";
			...
			if ("control" == c.nodeName && $b) {
				(e = c.getAttributeNode("simulatemass")) &&
				(gc = 1 == e.nodeValue);
				...
				if (e = c.getAttributeNode("sensitivity")) gb = 1 * e.nodeValue,
				1 > gb && (gb = 1);
			...
			if ("hotspots" == c.nodeName)
				for (f = c.firstChild; f;) {
					if ("label" == f.nodeName) {
						...
						if (e = f.getAttributeNode("borderalpha")) k.Q =
							1 * e.nodeValue;
			...
			if ("sounds" == c.nodeName || "media" == c.nodeName)
				for (f = c.firstChild; f;) {
					if ("sound" == f.nodeName) {
						...
						if (e = f.getAttributeNode("mode")) n.mode =
							Number(e.nodeValue);
					...
					if ("video" == f.nodeName) {
						...
						if (e = f.getAttributeNode("field")) n.field =
							Number(e.nodeValue);
						if (e = f.getAttributeNode("ambientlevel")) n.ca = Number(e.nodeValue);
						if (e = f.getAttributeNode("pan")) n.pan = Number(e.nodeValue);
						if (e = f.getAttributeNode("tilt")) n.tilt = Number(e.nodeValue);
						...
						if (e = f.getAttributeNode("fov")) n.J =
							Number(e.nodeValue);
		...
		if (h) {
			for (f = 0; 6 > f; f++)(e =
				h.getAttributeNode("tile" + f + "url")) && (Wb[f] = new String(e.nodeValue));
		...
		yb &&
		(ga = Ca = r, na = document.createElement("canvas"), na.width = 100, na.height = 100, na.id = "dummycanvas", s.appendChild(na), Aa());
		ga && g && (md(Gb), nd());
	...
	b.openNext = function(a, d) {
		...
		K();
		if ("{" == a.charAt(0))
			if (f = a.substr(1, a.length -
					2), ob[f]) b.Ca(ob[f], c), p(a);
			else {
