{if empty($path)}
	...
	{/if}
{/if}
{?$pageTitle = 'Аренда элитной недвижимости в Санкт-Петербурге от компании Вячеслава Малафеева «М16-Недвижимость» '}
{?$pageDescription = 'Долгосрочная аренда элитных квартир, апартаментов, пентхаусов, загородных домов в Санкт-Петербурге и ЛО. Историческая часть города и престижные пригороды, персональный менеджер в круглосуточном режиме доступа и сервис премиум-класса.'}
{?$gravity = array(
		'TL' => 'top left',
		'T' => 'top center',
		'TR' => 'top right',
						...
						<div class="item-offers">
							...
							{if !empty($item.price)}
								<div class="price">
									...
									<div class="full_price">{$item.properties.price.value} {$lang->get('тыс руб.', 'mln rub.')}</div>
							...
							<div class="swiper-slide m-vw flat-wrap{if $smarty.foreach.sim_n.total == 1} m-center{elseif !empty($smarty.foreach.sim_n.total) && $smarty.foreach.sim_n.total < 3} m-margin{/if}">
								...
								<div class='params'>
									...
									{if !empty($sim.properties.price_meter_from.value)}
										<div class="area">{$sim.properties.price_meter_from.value} <i>+</i><br><span>За месяц</span></div>
									{/if}
						...
						<div class="flat-wrap">
							...
							<div class="item-offers">
								...
								<div class="column">
									<div class="col-inner">
										{if !empty($item.price)}
											<div class="price">{$item.properties.price.value} {$lang->get('тыс руб.', 'mln rub.')}</div>
											{if !empty($item.area_all)}
												<div class="small-descr">{($item.price/$item.properties.area_all.value*1000)|ceil} тыс. руб за месяц</div>
											{/if}
										{else}
											<div class="price m-noprice"><div>{$lang->get('по запросу', 'on request')}</div></div>
											<a href="{$url_prefix}/arenda/request/?id={$complex.id}" class="small-descr">{$lang->get('Узнать цену','Find out price')}</a>
										{/if}
									</div>
								</div>
							</div>
