{if $request_segment.key == 'ru'}
	...
	{?$apartment_title = 'Элитная ' . $item.bed_number . '-комнатная квартира общей площадью ' . $item.area_all . (!empty($item.district) ? ', ' . $item.district.title : '')}
...
{*    {if $item.bed_number == 1}
        {?$pageTitle = "Элитные однокомнатные квартиры СПб, вторичный рынок | Продажа квартир от «М16-Недвижимость"}
        {?$pageDescription = "Хотите купить элитную однокомнатную квартиру в Санкт-Петербурге? Лучшие предложения вторичного рынка – только в агентстве «М16-Недвижимость»! Каталог квартир с ценами, фото, планировками и подробным описанием."}
    {elseif $item.bed_number == 2}
        {?$pageTitle = "Элитные 2-комнатные квартиры в разных районах СПб | Агентство недвижимости Вячеслава Малафеева"}
        {?$pageDescription = "Продажа элитных двухкомнатных квартир в разных районах Санкт-Петербурга. Цены, планировочные решения, фото, описания – в карточках объектов. У нас вы можете купить двухкомнатную квартиру на вторичном рынке в сегменте «бизнес» и «элит». "}
    {elseif $item.bed_number == 3}
        {?$pageTitle = "Купить 3-комнатную квартиру в СПб | Агентство элитной недвижимости «М16»"}
        {?$pageDescription = "Продажа трехкомнатных квартир в разных районах Санкт-Петербурга. Лучшие предложения вторичного рынка: широкий выбор планировок, метража, локации. В каталоге представлены цены, фото и описание элитных квартир."}
    {elseif $item.bed_number == 4}
        {?$pageTitle = "4-комнатные квартиры повышенной комфортности в СПб | Элитная недвижимость от «М16»"}
        {?$pageDescription = "Планируете купить четырехкомнатную квартиру в Санкт-Петербурге? Каталог актуальных предложений на вторичном рынке: цены, фото, разнообразные планировки. Продажа 4-комнатных квартир в разных районах города."}
    {elseif $item.bed_number == 5}
        {?$pageTitle = "Продажа 5-комнатных квартир от собственника | Элитная недвижимость в агентстве Вячеслава Малафеева"}
        {?$pageDescription = "Хотите купить 5-комнатную квартиру в Санкт-Петербурге от собственника? Наш каталог пяти- и шестикомнатных квартир включает в себя подробные фото, описание, планировки и цены. Квартиры в разных районах города. "}
	{/if}*}
...
<div class="top-bg" id="site-top">
	<a href="{$url_prefix}/resale/" class="back">{fetch file=$path . "arrow.svg"}</a>
		...
		<div class="gallery-small">
			<div class="flat-wrap m-vw m-gallery">
								...
								{foreach from=$gallery item=img name=img_n}
									...
									{if $smarty.foreach.img_n.iteration == 1 && !empty($item.video)}
										<div class="swiper-slide video-thumb">
											<img src="http://img.youtube.com/vi/{$item.video}/3.jpg" alt="">
										</div>
									{/if}
								{/foreach}
							</div>
				...
				<div class='params'>
					...
					<div class="price">
						<div class="full_price">{if !empty($item.price)}{$item.properties.price.value} {$lang->get('млн руб.', 'mln rub.')}{else}Узнайте цену{/if}</div>
						<div class="price_per_m">{if !empty($item.properties.area_all.value) && !empty($item.price)}{($item.price/$item.properties.area_all.value*1000)|ceil} {$lang->get('тыс. руб. за м', 'ths rub. per m')}<sup>2</sup>{else} {$lang->get('по запросу', 'by request')}{/if}</div>
					</div>
					<a href="{$url_prefix}/resale/request/?id={$item.id}" class="btn m m-magenta-fill m-vw">{$lang->get('Оставить заявку', 'Send your request')}</a>
				</div>
			...
			<div class="floor-wrap">
				...
				<a target="_blank" href="#" class="btn m-sand m-vw img-cont" style="line-height: 28px;color: #000; margin-bottom: 30px;">видео</a>
									...
									<iframe id="video-frame" src="http://www.youtube.com/embed/{$item.video}?autoplay=0&rel=0&theme=dark&enablejsapi=1&showinfo=0"" width="100%" height="100%" align="left">
</iframe>
					...
					{if !empty($item.shemes) && !empty($schemes)}
						...
						{/if}
					{else}
						<a href="{$url_prefix}/resale/request/?id={$item.id}" class="price">
		...
		<div class="main-specs row a-justify consultant">
			...
			<div class="opinion w1">
				...
				{if !empty($consultant_text)}
					...
					<a href="{$url_prefix}/resale/request/?id={$item.id}" class="btn m-white">{$lang->get('Задать вопрос', 'Ask a question')}</a>
				{/if}
			</div>
			<div class="person w1">
				{if !empty($consultant)}
...
<div class="qr-block-wrap">
	<div class="qr-block">
		<div class="qr">
			<div class="qr-wrap">
				<img src="https://chart.googleapis.com/chart?cht=qr&chs=235x235&chld=L|2&chl=http://{$smarty.server.SERVER_NAME}{$item->getUrl()}" alt="" />
			</div>
		</div>
