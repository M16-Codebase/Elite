		...
		{foreach from=$items item=item}
			...
			<div class="quick-item flat-wrap item-{$item.id}{if $item.id == $quick_view_current_id} m-current{/if}">
				...
				<div class="quick-title">
					{$item.title} 
					<span class="favorite-btn{if $item.in_favorites} m-added{/if} a-inline-block" data-id="{$item.id}" data-url="resale">{fetch file=$path . "favorite.svg"}</span>
				</div>
					...
					<div class="col2">
						...
						<div class="opt-table buttons">
							<div class="opt-row">
								...
								<div class="opt-col">
									<a href="{$url_prefix}/resale/request/?id={$item.id}" class="btn m-magenta-fill">{$lang->get('Оставить заявку', 'Send your request')}</a>
								</div>
							</div>
						</div>
					</div>
