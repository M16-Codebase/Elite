{if !empty($search_properties)}
	{foreach from=$search_properties item=sprop name=sprop}
				...
				{if !empty($foreign_price) && $sprop.key == 'close_price'}
					<div class="field tabs-cont">
						<div class="f-title main">
							{$lang->get("Цена","Price")}
							<span class="tab-title{if $request_segment.key == 'ru'} m-current{/if}" data-target=".js-rub-show"> {$lang->get("млн руб.","mln rub.")}</span><span class="slash"></span>
							<span class="tab-title{if $request_segment.key != 'ru'} m-current{/if}" data-target=".js-doll-show"> {$lang->get("тыс USD","ths USD")}</span>
						</div>
						<div class="f-input slider-wrap js-rub-show tab-page{if $request_segment.key == 'ru'} m-current{else} a-hidden{/if}">
							<input type="text" name="{$sprop.key}[min]" class="input-min range-input a-left" maxlength="3" value="{$sprop.search_values.min|floor}"/>
							<input type="text" name="{$sprop.key}[max]" class="input-max range-input a-left" maxlength="3" value="{$sprop.search_values.max|ceil}"/>
							<div class="slider range" data-min="{$sprop.search_values.min|floor}" data-max="{$sprop.search_values.max|ceil}" data-step="{$sprop.search_values.step || 1}"></div>
						</div>
						{if !empty($foreign_price) && $sprop.key == 'close_price'}
							<div class="f-input slider-wrap js-doll-show m-b-input tab-page{if $request_segment.key != 'ru'} m-current{else} a-hidden{/if}">
								<input type="text" name="{$foreign_price.key}[min]" class="input-min range-input a-left" maxlength="4" value="{$foreign_price.min|floor}"/>
								<input type="text" name="{$foreign_price.key}[max]" class="input-max range-input a-left" maxlength="4" value="{$foreign_price.max|ceil}"/>
								<div class="slider range" data-min="{$foreign_price.min|floor}" data-max="{$foreign_price.max|ceil}" data-step="{$foreign_price.step || 1}"></div>
							</div>
						{/if}
					</div>
				{else}
			...
			{if !empty($sprop.search_values)}
				{if $sprop.key== 'district'}
					<div class="field f-dropdown" data-hoverable="0">
						<div class="f-title main">{$sprop_title}<a href="{$url_prefix}/district/"> — {$lang->get('Гид по районам', 'City guide')}</a></div>
						...
						<ul class="dropdown-menu">
							{foreach from=$sprop.search_values item=sval_view key=val}
								<li>
									<input id="{$sprop.key . $val}" type="checkbox" name="{$sprop.key}[]" value="{$val}">
				...
				{elseif $sprop.key== 'typerk'}
					<div class="field f-dropdown" data-hoverable="0">
						<div class="f-title main">{$sprop_title}</div>
						<div class="dropdown-toggle"><span data-title="{$lang->get('все типы', 'all types')}" data-title_one="{$lang->get('тип', 'type')}" data-title_two="{$lang->get('типа', 'type')}" data-title_five="{$lang->get('типов', 'types')}">{$lang->get('все типы', 'all types')}</span><div>{fetch file=$path . "menu.svg"}</div></div>
						<ul class="dropdown-menu">
					...
					<div class="field">
						...
						<div class="f-input">
							...
							{foreach from=$sprop.search_values item=sval_view key=val name=sval}
								...
								{/if}
								<label class="btn m-bedroom">
									<input type="checkbox" name="{$sprop.key}[]" value="{$val}" class="m-hidden-input">
									<span>{$sval_view}</span>
								</label>
							{/foreach}
							{if $many_bed}
