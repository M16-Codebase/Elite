...
{if !empty($mode)}
	...
	{elseif $mode == 'resale_single'}
		{if !empty($apartment)}
			...
			{if ($apartment.typerk=='Квартира')}{?$main = $lang->get('Элитная квартира в Санкт-Петербурге', 'Luxury apartment in St.Petersburg')}{else}{?$main = $lang->get('Элитный коттедж в Санкт-Петербурге', 'Luxury cottage in St.Petersburg')}{/if}
	...
	<div class="request-wrap row">
		...
		<div class="w2">
			<div class="small-descr">
				{$lang->get('Наши специалисты свяжутся с вами, ответят на ваши вопросы, организуют просмотр квартиры и проведут сделку на высшем уровне', 'Our specialists will contact you, answer your questions, organize vizit to apartments and carry out the transaction at the highest level.')}
			</div>
								...
								<textarea type="text" name="message">
									{if !empty($mode)}
										...
										{elseif $mode == 'appartments_list'}
											...
											{/foreach}
										{elseif $mode == 'resale_single'}
										{if ($apartment.typerk=='Квартира')}
											{$lang->get('Интересует квартира', 'Interested in apartment')}{else}{$lang->get('Интересует коттедж', 'Interested in cottage')}{/if} «{if !empty($apartment.area_all)}{$apartment.area_all}{/if}» {$lang->get('по адресу', 'by address')} {if !empty($apartment.address)}{$apartment.address}{/if}
										{/if}
									{/if}
								</textarea>
							</div>
