{if $request_segment.key == 'ru'}
	{?$apartment_title = 'Элитный коттедж общей площадью общей площадью ' . $item.area_all . (!empty($item.district) ? ', ' . $item.district.title : '')}
...
{else}
	{?$apartment_title = 'Elite cottage with total area of ' . $item.area_all . (!empty($item.district) ? ', ' . $item.district.title : '')}
...
<div class="top-bg" id="site-top">
	<a href="{$url_prefix}/residential/" class="back">{fetch file=$path . "arrow.svg"}</a>
	<div class='bg-img' style='background: url(/img/veil.png), url({!empty($cover) ? $cover->getUrl() : ''});background-size:cover;'></div>
	<div class="site-top">
		<div class="main">{$lang->get('Элитный коттедж в Санкт-Петербурге', 'Luxury cottage in St.Petersburg')}</div>
		...
		<div class="gallery-small">
			<div class="flat-wrap m-vw m-gallery">
				...
				<div class='params'>
					...
					<a href="{$url_prefix}/residential/request/?id={$item.id}" class="btn m m-magenta-fill m-vw">{$lang->get('Оставить заявку', 'Send your request')}</a>
				</div>
			...
			<div class="floor-wrap">
				...
				<a target="_blank" href="#" class="btn m-sand m-vw img-cont" style="line-height: 28px;color: #000; margin-bottom: 30px;">видео</a>
					...
					{if !empty($item.shemes) && !empty($schemes)}
						...
						{/if}
					{else}
						<a href="{$url_prefix}/residential/request/?id={$item.id}" class="price">
	...
	{if !empty($item.features)}
		<div class="item-benefits row a-justify">
			<div class="ben-title main m-vw w2">
                {if ($item.typerk=='Таунхаус')}
                {$lang->get('Достоинства таунхауса', 'Advantages of the appartments')}
                {else}
		...
		<div class="main-specs row a-justify consultant">
			...
			<div class="opinion w1">
				...
				{if !empty($consultant_text)}
					...
					<a href="{$url_prefix}/residential/request/?id={$item.id}" class="btn m-white">{$lang->get('Задать вопрос', 'Ask a question')}</a>
				{/if}
			</div>
			<div class="person w1">
				{if !empty($consultant)}
...
<div class="post-map row a-justify{if $empty_description} m-center{/if}">
	...
	<div class="contacts-map w2">
			...
			<div class="map-big">
				...
				<div class="infoblock-content a-hidden">
					<div class="map-item-content">
						<div class="item-type main">{$lang->get('Элитный коттедж', 'Elite cottage')}</div>
		...
		{/if}
		<div class="map-small">
			<h2 class="main-top main" title="{$lang->get('Расположение и окружение квартиры', 'Apartment location and surroundings')}">{$lang->get('Расположение<br>и окружение коттеджа', 'Cottage location<br>and surroundings')|html}</h2>
			<div class="titles">
...
<div class="make-request">
	...
	<form action="/feedback/makeRequest/" class="request-form user-form" data-checkstring="{$checkString}" data-hashstring="{md5($checkStringSalt)}">
		...
		<div class="main m-vw open-block">{$lang->get('Отправьте заявку на просмотр коттеджа', 'Fill the form to send your booking request')}</div>
