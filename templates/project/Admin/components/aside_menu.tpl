	...
	{if $account->isPermission('seo')}
		<li class="am-submenu-contains{if $currentModuleUrl == 'seo' || $currentModuleUrl == 'config-seo' || !empty($seoPage)}  m-current m-open{/if}">
			...
			<ul class="aside-submenu">
                ...
                <li{if $currentModuleUrl == 'seo' && $action == 'seo-districts'} class="m-current"{/if}>
					<a href="/catalog-type/catalogIndex/?key=filter_seo" class="am-submenu-title">Seo для фильтров</a>
				</li>
				<li{if $currentModuleUrl == 'seo' && $action == 'seo-districts'} class="m-current"{/if}>
					<a href="/supportSeoLinks/" class="am-submenu-title">Вспомогательные сео-ссылки</a>
				</li>
            </ul>
		</li>
	{/if}
