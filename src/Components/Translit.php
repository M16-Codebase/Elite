...
class Translit
{
    ...
    //URL transliterating
    static function UrlTranslit($string, $allow_slashes = 0)
    {
        $slash = "";
        ...
        //here we replace ъ/ь
        $string = preg_replace("/(ь|ъ)([аеёиоуыэюя])/", "j\\2", $string);
        $string = preg_replace("/(ь|ъ)/", "", $string);
        //transliterating

        $string = str_replace(array_keys($Letters), array_values($Letters), mb_strtolower($string, 'utf-8'));
    ...
    //Supertag cooking
    static function Supertag($string, $allow_slashes = self::TR_NO_SLASHES)
    {
        $slash = "";
        if ($allow_slashes) $slash = "\/";
    ...
    static function BiDiTranslit($string, $direction = self::TR_ENCODE, $allow_slashes = self::TR_NO_SLASHES)
    {
        $slash = "";
        if ($allow_slashes) $slash = "\/";
        ...
        if ($direction == self::TR_ENCODE) {
            ...
            $string = implode(($allow_slashes != self::TR_NO_SLASHES) ? "/" : "", $pgs);
        }
        return rtrim($string, "/");
    }
    ...
    static function Wikify($string, $allow_slashes = self::TR_NO_SLASHES)
    {
        $slash = "";
        if ($allow_slashes) $slash = "\/";
    ...
    static function Dewikify($string)
    {
...
//echo Translit::UrlTranslit('Открываю тестовые новости');
?>