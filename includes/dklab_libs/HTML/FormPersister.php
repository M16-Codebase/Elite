 ...
 * @version 1.112 modified for LPS
 * @package HTML
 */
...
class HTML_FormPersister extends HTML_SemiParser
{
    ...
    function getCurValue($attr, $isBoolean = false)
    {
        ...
        if ($isArrayLike) {
            ...
            if ($isBoolean) {
                ...
                //pochepochka fix тут может быть и массив, если хитрые формы
                return (bool)@strval(is_array($value) ? '' : $value) === (bool)$attrValue;
            } else {
                // This is not boolean nor array field. Return it now.
                return @strval(is_array($value) ? '' : $value);
            }
        }
    }
     ...
     */
    static protected function _splitMultiArray($name) // static
    {
