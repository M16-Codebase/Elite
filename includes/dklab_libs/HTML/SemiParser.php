...
class HTML_SemiParser
{
     ...
     */
    public $sp_tags = array(); // array(tagName => list( h1, h2, ...), ...)
    public $sp_cons = array();
    public $sp_res = array();
    public $sp_precachers = array();
    public $sp_postprocs = array();
    public $sp_preprocs = array();
    
    /**
     * Functions for quoting/dequoting.
     */
    public $sp_quoteHandler = null;
    public $sp_dequoteHandler = null;

    /**
     * Object-callback name prefixes.
     */
    public $sp_preTag = "tag_";        // for tag gandlers
    public $sp_preCon = "container_";  // for container handlers
    public $sp_preRe  = "re_";         // for REs

    /**
     * Characters inside tag RE (between < and >).
     */
    public $sp_reTagIn = '(?>(?xs) (?> [^>"\']+ | " [^"]* " | \' [^\']* \' )* )';
    
    /**
     * Containers, whose bodies are not parsed by the library.
     */
    public $sp_IGNORED = array('script', 'iframe', 'textarea', 'select', 'title');
    public $sp_SKIP_IGNORED = true; 

    /**
     * Local temp publiciables.
     */
    public $sp_replaceHash;     // unique hash to replace all the tags
     ...
     */
    static function _unhtmlspecialchars($value)
    {
