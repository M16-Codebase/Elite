...
function quicky_block_select($params,$content,$compiler)
{
 ...
 if (isset($params['join']))
 {
  ...
  if (!isset($compiler->block_props['form'])) {return $compiler->_syntax_error('Parameter \'join\' in tag select must be into a form.');}
  $form = Form::$forms[$compiler->block_props['form'][0]];
  ...
  {
   $t = gettype($v);
   if ($t == 'string') {$content .= '{option text=\''.$compiler->escape_string($v).'\'}'."\n";}
   elseif ($t == 'array' or $t == 'object')
   {
    if ($t == 'array') {$v = (object) $v;}
    $type = isset($v->type)?$v->type:'option';
    unset($v->type);
    if ($type != 'option' and $type != 'optgroup') {return $compiler->_syntax_error('Unexcepted type of dropdown\'s child element: \''.$type.'\'');}
    $s = '';
    foreach ($v as $h => $b)
    {
