...
class Quicky_form
{
 ...
 {
  if ($this->_num_of_errors) {return FALSE;}
  if (isset($this->elements->submit) && !$this->elements->submit->clicked()) {return FALSE;}
  return TRUE;
 }
...
class QSubmit extends Quicky_form_element
{
 public function __construct($properties = array())
 {
  if (!isset($properties['type'])) {$properties['type'] = 'submit';}
  foreach ($properties as $k => $v) {$this->$k = $v;}
 }
 public function clicked()
 {
...
class QFile extends Quicky_form_element
{
 ...
 public function isUploaded() {return isset($this->name) && isset($_FILES[$this->name]['tmp_name']) && is_uploaded_file($_FILES[$this->name]['tmp_name']);}
 public function moveFileTo($dest) {return $this->isUploaded() && move_uploaded_file($_FILES[$this->name]['tmp_name'],$dest);}
 public function moveTo($dest) {return $this->moveFileTo($dest);}
 public function getContents($empty = FALSE) {return $this->isUploaded()?file_get_contents($_FILES[$this->name]['tmp_name']):($empty?'':FALSE);}
 public function getPath() {return $this->isUploaded()?$_FILES[$this->name]['tmp_name']:FALSE;}
 public function getName() {return isset($_FILES[$this->name]['name'])?$_FILES[$this->name]['name']:'';}
}
