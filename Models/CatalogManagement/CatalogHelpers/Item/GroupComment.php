...
class GroupComment extends ItemHelper{
    protected static $i = NULL;
	const TABLE = 'item_group_comments';
    ...
    private function loadData(){
        ...
        $comments = $db->query('SELECT * FROM `'.static::TABLE.'` WHERE `item_id` IN (?i)', $this->loadItemsQuery)->getCol(array('item_id', 'group_id', 'segment_id'), 'comment');
		if (!empty($comments)){
    ...
    public function onUpdate($updateKey, Item $item, $segment_id, $updatedProperties){
		...
		if (!empty(self::$additional_data[$item['id']][self::FIELD_NAME])){
			...
			if ($old_data != self::$additional_data[$item['id']][self::FIELD_NAME]){
				foreach (self::$additional_data[$item['id']][self::FIELD_NAME] as $g_id => $data){
					if (!empty($data) && (empty($old_data[$g_id]) || $data != $old_data[$g_id])){
						foreach ($data as $s_id => $comment){
							if (empty($old_data[$g_id][$s_id]) || $comment != $old_data[$g_id][$s_id]){
								$db->query('REPLACE INTO `'.self::TABLE.'` SET `item_id` = ?d, `group_id` = ?d, `segment_id` = ?d, `comment` = ?s', $id, $g_id, !empty($s_id) ? $s_id : 0, $comment);
							}
						}
					}
				}
